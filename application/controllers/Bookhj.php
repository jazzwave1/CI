...
class Bookhj extends CI_Controller {
   ...
   /* 
    ...
    {
        if(!$aBResult = $this->ebook_model->getBestBook())
        {
            // no data
            // 기획이 필요합니다.
            // 검색범위를 넓혀서 검색을 합니다.
        ...
        {
            $data = array();
            $aViewDataNewContents['bestlist'] = $aBResult;
        }
        
        if(!$aNResult = $this->ebook_model->getNewBook())
        {
            // no data
            // 기획이 필요합니다. 
        }
        ...
        {
            $data = array();
            $aViewDataNewContents['newlist'] = $aNResult;
        }
        ...
        $data['isLogin'] = $mb_id; 
        $data['contents'] = $this->load->view('book/book_list', $aViewDataNewContents, true) ;
